 ...
 */
function gutenberg_render_nested_page_list( $nested_pages, $active_page_ancestor_ids = array() ) {
	...
	foreach ( (array) $nested_pages as $page ) {
		...
		if ( isset( $page['children'] ) ) {
			...
			$markup .= '<ul class="submenu-container">' . gutenberg_render_nested_page_list( $page['children'], $active_page_ancestor_ids ) . '</ul>';
		}
		$markup .= '</li>';
	}
	return $markup;
...
function gutenberg_render_block_core_page_list( $attributes, $content, $block ) {
	...
	$items_markup = gutenberg_render_nested_page_list( $nested_pages, $active_page_ancestor_ids );
